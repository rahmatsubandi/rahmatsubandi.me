...
const url = "https://rahmatsubandi.me/";
...
export default function Index() {
  ...
  return (
    <>
      ...
      <Box h={1} as="div" bgGradient="linear(to-r, blue.600, blue.400, green.400, green.600)" position="sticky" transition="all 200ms linear" top={0} zIndex={100} w={`${width}%`}></Box>

      <Container>
        <Head><title>Home - Rahmat Subandi</title></Head>
        <Stack as="main" spacing={8} justifyContent="center" alignItems="flex-start" m="0 auto 4rem auto" maxWidth="800px" px={2}>
          <motion.div initial={{ y: -20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.7, delay: 0.4 }}>
            <Flex flexDirection="column" justifyContent="flex-start" alignItems="flex-start" maxWidth="800px">
              <Box as="section" mt={10} mb={10}>
                <Heading letterSpacing="tight" mt={8} size="lg" fontWeight={700} as="h2" mb={4}>About Me</Heading>
                <Text color={colorSecondary[colorMode]}>I study at
                  <Link href="https://www.mercubuana.ac.id/en" color={linkColor[colorMode]} isExternal>{" "}Mercu Buana University{" "}</Link>
                  with an undergraduate study program majoring in Informatics Engineering.. My personal website is where I showcase my
                  projects, writing, statistics, experience, and more. It also serves as a sandbox to play around with new technologies!
                  Feel free to reach out via{" "}
                  <Link href="mailto:rhmtin12@gmail.com" color={linkColor[colorMode]}>email</Link>{" "}or any social media with any inquiries.</Text>
              </Box>
              <Box as="section" mt={10} mb={20}>
                <Heading letterSpacing="tight" mt={8} mb={4} size="lg" fontWeight={700} as="h2">Top Repository GitHub 🐱‍💻</Heading>
              ...
              <NowPlaying />
              <Text mt={8} mb={4}>Looks like you've made it to the end of this page... feel free to check out my{" "}
                <NextLink href="/hello" passHref>
                  <Link isExternal color={linkColor[colorMode]}>/hello</Link>
                </NextLink>{" "}
